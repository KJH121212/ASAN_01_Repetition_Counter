...
DOCKER_CONTAINER_NAME="tojihoo_metadata-update"
RANDOM_PORT=$(( (RANDOM % 101) + 8888 ))  # Random port between 8888 and 8988

# ✅ Docker build
docker build \
  -t ${DOCKER_IMAGE_NAME} \
  -f /mnt/nas203/ds_RehabilitationMedicineData/IDs/tojihoo/ASAN_01_Repeatition_Counter/docker/Dockerfile.sapiens \
  /mnt/nas203/ds_RehabilitationMedicineData/IDs/tojihoo/ASAN_01_Repeatition_Counter/docker
...
docker run -it --rm --device=nvidia.com/gpu=all --shm-size 1TB \
    ...
    ${DOCKER_IMAGE_NAME} \
    bash -c "
        cd /workspace/nas203/ds_RehabilitationMedicineData/IDs/tojihoo/ASAN_01_Repeatition_Counter/runner && \
